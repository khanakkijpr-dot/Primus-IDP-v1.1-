...
export function ModelConfigManager({ searchSpaceId }: ModelConfigManagerProps) {
	...
	const handleProviderChange = (providerValue: string) => {
		...
		setFormData((prev) => ({
			...
			// Auto-fill model name for Ollama
			model_name: providerValue === "OLLAMA" ? defaultModel : prev.model_name,
		}));
	};
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (editingConfig) {
			...
			result = await updateLLMConfig(editingConfig.id, submitData);
		} else {
			// Create new config
			result = await createLLMConfig(submitData);
		}
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						...
						<div className="space-y-2">
							<Label htmlFor="api_key">
								{formData.provider === "OLLAMA" ? "API Key (Optional)" : "API Key *"}
							</Label>
							<Input
								id="api_key"
								type="password"
								placeholder={formData.provider === "OLLAMA" ? "Leave empty for Ollama" : "Your API key"}
