...
description: Setting up Primus IDP using Docker
full: true
---
...
This guide explains how to run Primus IDP using Docker Compose, which is the preferred and recommended method for deployment.
...
1. **Configure Environment Variables**
    ...
    **Linux/macOS:**
    ...
    cp Primus IDP_backend/.env.example Primus IDP_backend/.env
    cp Primus IDP_web/.env.example Primus IDP_web/.env
    ...
    **Windows (Command Prompt):**
    ...
    ```cmd
    copy Primus IDP_backend\.env.example Primus IDP_backend\.env
    copy Primus IDP_web\.env.example Primus IDP_web\.env
    ...
    **Windows (PowerShell):**
    ...
    ```powershell
    Copy-Item -Path Primus IDP_backend\.env.example -Destination Primus IDP_backend\.env
    Copy-Item -Path Primus IDP_web\.env.example -Destination Primus IDP_web\.env
...
| POSTGRES_DB                | PostgreSQL database name                                                    | Primus IDP           |
| PGADMIN_DEFAULT_EMAIL      | Email for pgAdmin login                                                     | admin@Primus IDP.com |
| PGADMIN_DEFAULT_PASSWORD   | Password for pgAdmin login                                                  | Primus IDP           |
...
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DATABASE_URL               | PostgreSQL connection string (e.g., `postgresql+asyncpg://postgres:postgres@localhost:5432/Primus IDP`)                                                                                    |
...
| LANGSMITH_PROJECT | LangSmith project name (e.g., `Primus IDP`) |
...
2. Login with the credentials from your `.env` file (default: admin@Primus IDP.com / Primus IDP)
3. Right-click "Servers" > "Create" > "Server"
4. In the "General" tab, name your connection (e.g., "Primus IDP DB")
5. In the "Connection" tab:
   - Host: `db`
   - Port: `5432`
   - Maintenance database: `Primus IDP`
...
Once your installation is complete, you can start using Primus IDP! Navigate to the frontend URL and log in using your Google account.
