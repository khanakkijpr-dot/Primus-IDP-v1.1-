...
description: Setting up Primus IDP manually for customized deployments (Preferred)
full: true
---

# Manual Installation (Preferred)

This guide provides step-by-step instructions for setting up Primus IDP without Docker. This approach gives you more control over the installation process and allows for customization of the environment.
...
The backend is the core of Primus IDP. Follow these steps to set it up:
...
```bash
# Make sure you're in the Primus IDP_backend directory
cd Primus IDP_backend
...
The Primus IDP browser extension allows you to save any webpage, including those protected behind authentication.
...
```bash
cd Primus IDP_browser_extension
cp .env.example .env
```
...
| ENV VARIABLE              | DESCRIPTION                                           |
| ------------------------- | ----------------------------------------------------- |
| PLASMO_PUBLIC_BACKEND_URL | Primus IDP Backend URL (e.g., `http://127.0.0.1:8000`) |
...
Load the extension in your browser's developer mode and configure it with your Primus IDP API key.
...
Now that you have Primus IDP running locally, you can explore its features:
